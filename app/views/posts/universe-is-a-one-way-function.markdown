...
Most of my acquaintances with a modest amount of scientific knowledge, of course, dismissed Steorn from the very beginning. What the company claims to do violates a known law of physics, they argued; therefore it is impossible; therefore they are lying (or confused). To be honest, I never did and probably never will fully sympathize with this attitude. While I definitely agree that Steorn probably do not have what they thought they had (and I certainly have no intention of arguing with the laws of thermodynamics!), I disagree with the premise that we can be *so sure* of things like this that we are justified in rejecting them immediately.

This is actually the same topic I covered in [my very first post on this blog](http://philosopherdeveloper.wordpress.com/2009/12/19/the-myth-of-the-myth-of-perpetual-motion/). But it's something I feel quite strongly about, so I'll probably write about it again, and again, until I'm satisfied I've fully covered the topic in the way I want. This time around, I want to relate my skepticism\* in these sorts of matters to the mathematical concept of *one-way-functions*. To begin, I'll start with a deceptively simple problem.
...
In Zendo, one player--the Master--devises a rule involving the game pieces, which he then illustrates via two examples: one embodying the rule, and one not. These examples are designated *true* and *false*. The players then take turns assembling their own game pieces in different formations, asking the master whether or not these formations embody the master's rule. Eventually, one or more of the players will *deduce* the rule by observing a *pattern* across the examples.

I was recently surprised\*\* when, during a recent game, after the master had given his initial two examples--each comprising four pieces of all different colors--my friend announced, "I have it already."

This declaration of having the answer so early in the game seemed absurd to me. And I think you can understand why, from my earlier thoughts on pattern recognition. There was no way my friend already *knew* the rule at this stage, given that there were many (in fact, *infinitely* many--that's a separate discussion) possible rules that would be consistent with the examples given.

Put another way: my friend had not narrowed down the possibilities enough to justify his feeling of certainty (at least to my satisfaction). He was guilty of **overeager deduction**.
...
This is in contrast to a *two-way* function, which is easy to reverse. An example of a two-way function would be multiplication. Given the function *y = 3x*, when we apply the *input* of 5, we simply multiply 3 &times; 5 and get the *output* 15. Likewise, if we know the *output* is 15, we divide 15 &div; 3 and conclude that the *input* must have been 5.

An example of a *one-way* function is [prime factorization](http://en.wikipedia.org/wiki/Prime_factorization). Take some modestly large non-prime number--say, 946,905,102,747. Can you tell me this number's prime factors (i.e., what prime numbers divide it evenly)?
...
Here, the function in question might be worded in plain English as: *These numbers all multiplied together equal what?* It is very easy to compute the *output* of this function given the above factors an *input*. The reverse might be worded: *This number equals what other numbers multiplied together?* From the *output*, figuring out the *input* is much harder.

### Data encryption

If we start with the "reversed" form of a one-way function, then, what we have is a problem that is easy to *verify* yet difficult to *solve*. It turns out that this is a highly useful device in software security. It is one of the underlying principles of data encryption.

One illustration of this is the way passwords are stored on most websites. If you create an account with my website, and I then save your password in my database, then a hacker who breaks into my system can easily read your password.

![Plaintext password](/images/plaintext_password.png)

But suppose instead of storing your *password*, I store the output of some cryptographic (one-way) function, using your password (and perhaps a [salt](http://en.wikipedia.org/wiki/Password_salting)) as input.
...
### Black boxes are a kind of one-way function

It might seem that I've veered off topic a bit. How are pattern recognition, Zendo, and one-way functions all related?

I believe the concept of a one-way function is much broader than you might think from my initial mathematical examples.

We can think of a *rule* in Zendo as its own kind of function. The input to this function is a formation of game pieces, and the ouput is either true or false.
...
Here's something else that has always sort of bothered me: detective stories. Have you ever noticed that in these stories, when the brilliant detective protagonist finally cracks the case, he generally reveals the full narrative of what happened to a room full of mesmerized listeners? In this narrative, he paints a vivid picture of how the villain prepared, all of the meticulous steps he took to avoid detection, and all of the little mistakes he made leaving clues that ultimately led the detective to expose him.

Here's why these stories--though, to be honest, I generally quite like them as entertainment--nonetheless fall short for me in terms of logical satisfaction. They are illustrations of what I will call the "Sherlock tendency", which is this: as human beings, **once we have identified a *plausible explanation* for some event, we think we have uncovered the truth**.

![The Sherlock tendency](/images/sherlock_tendency.png)

In the abstract, this is not any different from the "overeager deduction" I accused my friend of in Zendo earlier. It is also the same as observing a sequence such as *1, 2, 4* and feeling certain at having recognized the pattern of which that sequence is a part.
